...
// Coin rewards (available coin items)
const coinItems = [
...
// Coupon rewards (available coupon items)
const couponItems = [
...
function getLevelMpReward(level) {
    // Early levels (10-19) - Small rewards
    ...
    // Levels 20-29 - Medium rewards
    if (level >= 20 && level <= 29) {
      return 7000;
    }
    
    // Levels 30-39 - Growing rewards
    ...
    // Levels 50-59 - Major milestone rewards
    if (level === 50) {
    ...
    // Levels 60-69 - High tier rewards
    if (level === 60) {
      return 30000; // Milestone bonus for level 60
    }
    if (level >= 61 && level <= 69) {
      return 9000;
    }
    
    // Levels 70-79 - Elite rewards
    if (level === 70) {
      return 10000; // Milestone bonus for level 70
    }
    if (level >= 71 && level <= 79) {
      return 9000;
    }
    
    // Levels 80-89 - Master rewards
    if (level === 80) {
      return 50000; // Milestone bonus for level 80
    }
    if (level >= 81 && level <= 89) {
      return 8000;
    }
    
    // Levels 90-99 - Grandmaster rewards
    if (level === 90) {
      return 100000; // Milestone bonus for level 90
    }
    if (level >= 91 && level <= 99) {
      return 9000;
    }
    
    // Levels 100+ - Legendary rewards
    if (level === 100) {
      return 100000; // Major milestone bonus for level 100
    }
...
const levelAchievements = levels.map(level => {
  ...
  // 1. Add MP reward (manual mapping)
